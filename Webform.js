...
var Webform = /*#__PURE__*/function (_NestedDataComponent) {
  ...
  _createClass(Webform, [{
    ...
    key: "setAlert",
    value: function setAlert(type, message, classes) {
      var _this14 = this;
      ...
      if (message) {
        this.alert = this.ce('div', {
          class: classes || "alert alert-".concat(type),
    ...
    value: function focusOnComponent(key) {
      if (key) {
        var path = (0, _utils.getArrayFromComponentPath)(key);
      ...
      errors.forEach(function (err) {
        if (err) {
          var createListItem = function createListItem(message, index) {
            ...
            if (keyOrPath) {
              var formattedKeyOrPath = (0, _utils.getStringFromComponentPath)(err.messages[index].path);
          ...
          if (err.messages && err.messages.length) {
            var errLabel = _this15.t(err.component.label);

            err.messages.forEach(function (_ref, index) {
              var message = _ref.message;
              return createListItem("".concat(errLabel, ". ").concat(message), index);
            });
          } else if (err) {
